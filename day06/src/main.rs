...
fn part2_distance_san_you(orbit_map: &HashMap<String, String>) -> i32 {
    distance_between_orbits(
        orbit_map,
        orbit_map.get("YOU").expect("should exist YOU").to_owned(),
        orbit_map.get("SAN").expect("Should exist SAN").to_owned(),
    )
}

fn distance_between_orbits(
...
fn part1_total_orbits(orbit_map: HashMap<String, String>) -> i32 {
    orbit_map.keys().fold(0, |acc, orbit| {
...
fn get_inputs() -> Vec<(String, String)> {
    let file = File::open("/Users/aalvarez/Projects/DiaCriativo/AdventOfCode2019/inputs/day06.txt")
